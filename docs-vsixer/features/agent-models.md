...
2. `<worktree>/.opencode/config/agent-models.jsonc`;
3. `<directory>/.opencode/config/agent-models.jsonc` (рабочая директория вне git);
4. `~/.config/opencode/config/agent-models.jsonc`.
...
Оверлей `agent-models.local.jsonc` рядом с найденным файлом: учитываются только
...
рядом с файлом, который ядро реально находит.
