...
- [`features/agent-models.md`](features/agent-models.md) — native `agent-models.jsonc` registry: role-based model/reasoning assignment for agents and commands (`prefix:путь` → `capability[:N]`), `builtin:` roles for `.md`-less agents, availability gates with provider groups, `agent-models.disabled.jsonc` overlay, optional `force` override, command reasoning pipeline (registry role beats agent reasoning and variants). Status: implemented, 2026-08-31.
